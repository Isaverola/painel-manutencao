...
def extrair_id_maquina(texto):
    """Extrai apenas o código numérico da máquina."""
...
def extrair_nome_maquina(texto):
    """Pega a parte do NOME da máquina (depois do hífen)."""
    if not isinstance(texto, str):
        return "Desconhecido"
    if ' - ' in texto:
        partes = texto.split(' - ')
...
def load_data():
    ...
    for chave, lista_nomes in arquivos_map.items():
        ...
        if arquivo_encontrado:
            try:
                dfs[chave] = pd.read_csv(arquivo_encontrado)
                if len(dfs[chave].columns) < 2:
                    dfs[chave] = pd.read_csv(arquivo_encontrado, sep=';')
            except:
...
if df_os is not None and not df_os.empty:
    ...
    # G3: Ranking de Máquinas (ALTERAÇÃO: Nome do Gráfico)
    ...
    # G1: Qtd O.S. (ALTERAÇÃO: Nome do Gráfico)
    with c1:
    ...
    # G3: Máquinas Distintas (ALTERAÇÃO: Nome do Gráfico)
    with c3:
        ...
        fig3.update_traces(hovertemplate='Mês: %{x}<br>Ativos: %{y}')
        st.plotly_chart(fig3, use_container_width=True)

    # G4: Falhas Totais (ALTERAÇÃO: Nome do Gráfico)
    with c4:
    ...
    # G5: Tempo Médio (ALTERAÇÃO: Nome do Gráfico)
    with c5:
        ...
        if 'Duracao_Horas' in df_os_filt.columns:
            ...
            fig5.update_traces(hovertemplate='Mês: %{x}<br>Média: %{y:.2f} h')
